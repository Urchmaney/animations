...
export default makeScene2D(function* (view) {
    ...
    const students: Rect[] = [];
    const studentPosition: SimpleSignal<number[], void>[] = range(8).map((i) => createSignal([-800 + 270 * i, 0]));

    const curves: QuadBezier[] = [];
    const backCurves: QuadBezier[]  = [];
    ...
    view.add(
        range(6).map((i) => (
            <QuadBezier
                ...
                endArrow
                stroke={'lightgreen'}
                p0={() => [studentPosition[i + 1]()[0] - 100, studentPosition[i + 1]()[1] + 50]}
                p1={[studentPosition[i + 1]()[0] - 50, 50]}
                p2={() => [studentPosition[i]()[0] + 100, studentPosition[i]()[1] + 50 ]}
                ref={makeRef(backCurves, i)}
                end={0}
            />
        ))
    )
    ...
    students.map((student, i) => student.add(
        <Txt layout fontSize={30} text={`${(i) < 1 ? 'none' : (i)}`} offset={[1, 0]} position={[-35, 70]} />
    ));

    students.map((student, i) => student.add(
        <Txt layout fontSize={30} text={`${(i + 2) > 7 ? 'none' : (i + 2)}`} offset={[0, 1]} position={[60, 90]} />
    ));



    yield* all(
        ...curves.map(c => c.end(2, 1))
    )

    yield* all(
        ...backCurves.map(c => c.end(2, 1))
    )

    const qcurves = view.findAll(is(QuadBezier));
    qcurves.forEach(c => c.remove());
    ...
    for (let i = 0; i < curves.length; i++) {
        yield* curves[i].end(2, .5);
    }

    for (let i = 0; i < curves.length; i++) {
        yield* backCurves[i].end(2, .5);
    }
    ...
    yield* all(
        ...
        ...backCurves.map((curve, i) => curve.p1.x(-700 + 270 * i + 100 , 2)),
        ...backCurves.map((curve, i) => curve.p1.y(50, 2)),
    );
    ...
    yield* all(
        ...
        ...backCurves.map((curve, i) => curve.p1.y(250, 2)),
    );
    ...
    yield* qText().text("What if we need to switch page 2 and 5 ?", 1).wait(1).back(.5);
    ...
    yield* all(
        ...
        students[2].findFirst<Txt>(t => (t as Txt).fontSize() === 30).text("", 2),
        curves[0].end(0, 1),
        curves[1].end(0, 1),
        backCurves[0].end(0, 1),
        backCurves[1].end(0, 1),


        ...secondStudent.findAll<Txt>(t => (t as Txt).fontSize() === 30).map(t => t.text("", 2)),
        students[3].findLast<Txt>(is(Txt)).text("", 2),
        students[5].findFirst<Txt>(t => (t as Txt).fontSize() === 30).text("", 2),
        curves[3].end(0, 1),
        curves[4].end(0, 1),
        backCurves[3].end(0, 1),
        backCurves[4].end(0, 1),
        ...fifththStudent.findAll<Txt>(t => (t as Txt).fontSize() === 30).map(t => t.text("", 2))
    )
    ...
    backCurves[3].p0(() => [studentPosition[1]()[0] - 100, studentPosition[1]()[1] + 50])
    backCurves[4].p2(() => [studentPosition[1]()[0] + 100, studentPosition[1]()[1] + 50]);
    backCurves[4].p1([-700 + 270 * 4 + 100, 0]);
    ...
    yield* all(
        ...
        curves[1].end(1, 1),
        ...students[1].findAll<Txt>(is(Txt)).slice(1).map((t, i) => t.text(["4", "6"][i], 2)),

        backCurves[3].end(1, 1),
        backCurves[4].end(1, 1),
        students[5].findFirst<Txt>(t => (t as Txt).fontSize() === 30 ).text("2", 2),
    );


    
    curves[0].p2(() => [studentPosition[4]()[0] - 100, studentPosition[4]()[1]])
    backCurves[0].p0(() => [studentPosition[4]()[0] - 100, studentPosition[4]()[1] + 50])
    curves[4].p2(() => [studentPosition[2]()[0] - 100, studentPosition[2]()[1]]);
    curves[4].p1([-700 + 270 * 1 + 100, 0]);

    backCurves[1].p2(() => [studentPosition[4]()[0] + 100, studentPosition[4]()[1] + 50])
     backCurves[1].p1([-700 + 270 * 1 + 100, 0]);
 

    yield* all(
        students[0].findLast<Txt>(is(Txt)).text("5", 2),
        curves[0].end(1, 1),
        backCurves[0].end(1, 1),
        ...students[4].findAll<Txt>(is(Txt)).slice(1).map((t, i) => t.text(["1", "3"][i], 2)),

        students[4].findLast<Txt>(is(Txt)).text("3", 2),
        curves[4].end(1, 1),
        backCurves[1].end(1, 1),
        students[2].findFirst<Txt>(t => (t as Txt).fontSize() === 30 ).text("5", 2),
    );
    ...
    yield* all(
        ...
        backCurves[1].p1([backCurves[1].p2().x, 250], 2),
        backCurves[4].p1([backCurves[4].p2().x, 250], 2)
    )   

    yield* all(
        qText().text("Double Linkedlist", 2).back(2),
    ...
    yield* all(
        qText().text("Single Linkedlist", 3).back(2),
        ...backCurves.map((curve, i) => curve.end(0, 2)),
        ...students.map(s => s.findFirst<Txt>(t => (t as Txt).fontSize() === 30).text("", 2)),
    )
})
...
const parsedSvg = parse(africaSvgString).children[0] as ElementNode;

const indexedCountries: Record<string, number> = parsedSvg.children.reduce((acc, x, index) => { acc[((x as ElementNode).properties["data-name"] as string)] = index; return acc} , {} as Record<string, number>)

const countryList = parsedSvg.children.map(x => (x as ElementNode).properties["data-name"] as string )
export { africaSvgString, countryList, indexedCountries };
import { SVG, SVGProps, Path, is, Segment, signal, CanvasStyleSignal, canvasStyleSignal, PossibleCanvasStyle, initial } from "@motion-canvas/2d";
...
const countryList = [
  ...
  "Côte d'Ivoire", 'Cameroon', 'Dem. Rep. Congo',
  'Congo', 'Djibouti', 'Algeria',
  'Egypt', 'Eritrea', 'Ethiopia',
  'Gabon', 'Ghana', 'Guinea',
  'Gambia', 'Guinea-Bissau', 'Equatorial Guinea',
  'Kenya', 'Liberia', 'Libya',
  'Lesotho', 'Morocco', 'Madagascar',
  'Mali', 'Mozambique', 'Mauritania',
  'Malawi', 'Namibia', 'Niger',
  'Nigeria', 'Rwanda', 'Western Sahara',
  'Sudan', 'South Sudan', 'Senegal',
  'Sierra Leone', 'Swaziland', 'Chad',
...
type ValueOf<T> =  T[keyof T & number] & string
type CountriesPaths = {
  [key in ValueOf<typeof countryList> as `focus${Capitalize<string & key>}`]?: () => Path
}

interface AfricaMapProps extends Omit<SVGProps, "svg"> {
  pathStrokeColor?: SignalValue<PossibleCanvasStyle>;
  pathStrokeWidth?: SignalValue<number>
}

export class AfricaMap extends SVG {
 
  private readonly paths: Path[];
  public readonly countries: CountriesPaths;

  @canvasStyleSignal()
  public declare readonly pathStrokeColor: CanvasStyleSignal<this>;

  @initial(1)
  @signal()
  public declare readonly pathStrokeWidth: SimpleSignal<number, this>;

  constructor(props: AfricaMapProps) {
    super({...props, svg: africaSvgString  });
    this.paths = this.findAll(is(Path));
    this.paths.forEach(path => {
      path.stroke(() => this.pathStrokeColor());
      path.lineWidth(() => this.pathStrokeWidth())
    })
    this.countries = {};
    this.setCountriesFocus();
  }

  private setCountriesFocus() {
    countryList.forEach((val: ValueOf<typeof countryList>, index: number) => {
      this.countries[`focus${val}`] = () => {
        const path = this.paths[index];
        const bbox = BBox.fromPoints(...path.profile().segments.flatMap((segment: Segment) => segment.points)).center;
        const center = this.size().div(new Vector2(2, 2));
        const newOffset = bbox.sub(center).div(center);
